...
function delNodes(root, to_delete) {
    ...
    delNodesAddRoots(root, toDeleteSet, roots, true);
    
    return roots;
}

function delNodesAddRoots(node, toDeleteSet, roots, isNewRoot) {
    if (node == null) {
        return null;
    }
    
    let isDeleted = toDeleteSet.has(node.val);
    node.left = delNodesAddRoots(node.left, toDeleteSet, roots, isDeleted);
    node.right = delNodesAddRoots(node.right, toDeleteSet, roots, isDeleted);
    
    if (!isDeleted && isNewRoot) {
        roots.push(node);
    }
