...
function isBalanced(root) {
    return isBalancedRecursively(root) !== -1;
}

function isBalancedRecursively(node) {
    if (node == null) {
        return 0;
    }
    
    let distLeft = Math.max(isBalancedRecursively(node.left));
    if (distLeft === -1) {
        return -1;
    }
    
    let distRight = Math.max(isBalancedRecursively(node.right));
    if (distRight === -1 || Math.abs(distLeft - distRight) > 1) {
        return -1;
    }
