...
function pathSum(root, sum) {
    return pathSumRec(root, sum, 0, new Map([[0, 1]]));
}

function pathSumRec(node, targetSum, curSum, prefixSumsMap) {
    if (node == null) {
        return 0;
    }
    ...
    let pathsToSumCount = 0;
    if (prefixSumsMap.get(curSum - targetSum) > 0) {
        pathsToSumCount += prefixSumsMap.get(curSum - targetSum);
    }
    
    prefixSumsMap.set(curSum, (prefixSumsMap.get(curSum) || 0) + 1);
    
    pathsToSumCount += pathSumRec(node.left, targetSum, curSum, prefixSumsMap) +
        pathSumRec(node.right, targetSum, curSum, prefixSumsMap);
    
    prefixSumsMap.set(curSum, prefixSumsMap.get(curSum) - 1);
    return pathsToSumCount;
}
