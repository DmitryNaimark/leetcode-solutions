...
function convertBST(root) {
    ...
    convertBSTRecursively(root, sharedObj);
    return root;
}

function convertBSTRecursively(node, sharedObj) {
    if (node == null) {
        return 0;
    }
    
    convertBSTRecursively(node.right, sharedObj);
    sharedObj.sum += node.val;
    node.val = sharedObj.sum;
    
    convertBSTRecursively(node.left, sharedObj);
}
