...
MaxStack.prototype.push = function(x) {
    if (this.stack.length === 0 || x > this.peekMax()) {
        this.maxStack.push(x);
    } else {
        let topElement = this.maxStack[this.maxStack.length - 1];
        this.maxStack.push(topElement);
    }
...
MaxStack.prototype.popMax = function() {
    ...
    for (let i = this.stack.length - 1; i >= 0; i--) {
        if (this.stack[i] === max) {
            ...
            ret = this.stack.splice(i, 1)[0];
            
            for (let j = i; j < this.stack.length; j++) {
                let previousMax = j > 0 ? this.maxStack[j - 1] : -Infinity;
                this.maxStack[j] = Math.max(previousMax, this.stack[j]);
            }
            break;
        }
    }
