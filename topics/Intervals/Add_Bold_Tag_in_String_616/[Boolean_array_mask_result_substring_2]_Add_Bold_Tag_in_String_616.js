...
function addBoldTag(s, dict) {
    ...
    for (let word of dict) {
        let iStart,
            // Used not to reassign same bolds[i] again (skip them).
            iLastBold = -1;
        
        while ((iStart = s.indexOf(word, iStart)) !== -1) {
            for (let i = Math.max(iStart, iLastBold); i < iStart + word.length; i++) {
                bolds[i] = true;
            }
            iStart++;
            iLastBold = iStart + word.length - 1;
        }
    }
    ...
    for (let i = 0; i < bolds.length; i++) {
        if (bolds[i] && (i === 0 || !bolds[i - 1])) {
            let iBoldEnd = i;
            while (bolds[iBoldEnd]) {
                iBoldEnd++;
            }
            
            result.push('<b>' + s.substring(i, iBoldEnd) + '</b>');
...
console.log(addBoldTag("",
